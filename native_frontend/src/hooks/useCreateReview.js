...
const useCreateReview = () => {
    ...
    onError: ({graphQLErrors, networkError}) => {
      ...
      if (networkError?.result?.errors) {
        networkError.result.errors.map(({ message, locations, extensions }) => {
          const msg = `[Network error]: Line ${locations[0].line}:${locations[0].column}: ${message} Extension: ${JSON.stringify(extensions)}`
  ...
  const createReview = async ({ repositoryOwner, repositoryName, repositoryRating, repositoryReview }) => {
    setErrors([]);
